...
This reference summarizes the user's current sniper/Alpha monitoring system as of 2026-07-17.
...
- Intraday flow watch scans large buys/sells, runtime CEX deposit candidates, CEX gas priming, report-only CEX internal aggregation, and report-only near-equal CEX withdrawal-cluster candidates whose direction remains `unknown`.
- Perp/OI/funding watch covers Binance/OKX/Bybit public derivatives context, normalizes actual settlement intervals to an 8-hour basis, separates current predicted funding from settled 24-hour history, and reports scope-matched aggregate OI changes without changing the primary-venue action rule.
...
- Treat Binance Alpha due-diligence CEX wallet aggregation as official discovery. Verify TXID, receipt, token, from/to, decimals, and the configured CEX destination before runtime use; classify an unlabeled source as `unlabeled_to_cex_inflow_candidate`, an explicit external source as `external_to_cex_inflow`, a CEX-internal transfer as `cex_internal_aggregation`, and any Alpha custody-related path as `alpha_custody_movement_unresolved`.
- `unlabeled_to_cex_inflow_candidate` is a CEX supply-risk input. It does not establish source entity, economic external inflow, project distribution, sale intent, next hop, or confirmed selling.
- Keep `cex_internal_aggregation` and `alpha_custody_movement_unresolved` at `direction=unknown`, `runtime_effect=none`, and `report_only`; count only external inflow attributed by ordered in-before-CEX-out matching when a later sweep reaches a hot wallet, require complete transfer coverage for runtime risk, and leave window-start inventory unresolved.
...
- Give every social/KOL setup one `root_signal_id`; merge self-quotes, follow-ups, and outcome posts into that case, and retain won, lost, mixed, and unresolved outcomes at fixed horizons.
- Recompute `regime_expectancy` from current liquidity, MC/FDV, scope-matched aggregate OI, venue policy, and capital concentration. Historical monster-token multiples do not set the current target.
- Keep claims pending when source publication time, claimed event time, or quote context conflicts.
...
- Do primary-venue OI and scope-matched aggregate OI agree, diverge, or lack comparable venue coverage?
...
- For tokenomics catalysts, is it burn, buyback, buyback-to-liquidity, fee donation, foundation, airdrop, initial float, utility change, mint, reissue, retirement, compensation, snapshot, or migration?
...
- What is the `root_signal_id`, which updates belong to it, and what are its fixed-horizon MFE, MAE, end return, invalidation, exit feasibility, and unresolved state?
- What is the current `regime_expectancy`, including target, staged exits, and time stop?
...
- Deduplicate quoted self-updates under one root signal before calculating outcomes; a repeated winning case contributes one denominator unit.
- Keep unresolved cases in the denominator and use fixed 24h, 72h, and 7d review horizons with MFE, MAE, and end return.
- Treat source-time contradictions as an evidence-quality failure until the original context is recovered.
- Expand the supply lifecycle vocabulary to mint, reissue, retirement, compensation, snapshot, and migration; verify relative float, first receiver, lock conditions, and the CEX/LP path.
- Treat wash-volume claims as report-only candidates until gross turnover, net-to-gross, repeated or round-trip addresses, and quote recovery are locally verified.
- Rebase target multiples and exits on current market structure; prior-cycle 10x or 100x examples remain context.
