...
3. For KOL/post research or external-agent material, request the project intake format in `/Users/xuyufan/Documents/狙击手进程/docs/kol_strategy_intake_prompt.md`.
4. Gather fresh source evidence if the update depends on public claims or current market behavior.
5. Add the smallest durable rule.
6. Mark source quality:
...
7. Keep raw transcripts out of the skill.
8. Store only reusable principles, case references, and operating rules.
9. Validate the skill after edits.
10. If the skill repo is available, commit and push to GitHub.
11. If the local Codex skill directory is separate from the repo, sync `celue` into `~/.codex/skills/celue`.
